...
module.exports = function(options) {
  ...
  function create(index) {
    ...
    worker.on('exit', function(code, signal) {
      logger.debug && logger.debug('WORKER #%d  ...stopped', worker.id);
      if (running) create(index);
    });

    logger.debug && logger.debug('WORKER #%d  ...started', worker.id, worker.process.pid);
  }
  ...
  function serve(server) {
    ...
    process.on('message', function(message, connection) {
      ...
      if (connection && message.length && message[0] === PREFIX + 'connection') {
        ...
        // log
        logger.debug && logger.debug('WORKER #%d  got conn from %s', id, connection.remoteAddress, message[1] && message[1].sticky);
  ...
  function entrust(index, connection, options) {
    var id = ids[index];
    logger.debug && logger.debug('MASTER  conn from %s goes to worker #%d', connection.remoteAddress, id);
