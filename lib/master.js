...
module.exports = function(workers, options) {
  ...
  function ipHashBalancer(connection) {
    ...
    logger.debug && logger.debug('MASTER  ip hash balancer receive connection from ' + signature);
  ...
  function stickyHeaderBalancer(connection) {
    ...
    const signature = connection.remoteAddress + ':' + connection.remotePort;
    logger.debug && logger.debug('MASTER  sticky header balancer receive connection from ' + signature);
    ...
    connection.once('data', function(buffer) {
      ...
      parser.onIncoming = function(req, res) {
        ...
        logger.debug && logger.debug('MASTER  sticky header balancer request:', req.url, res);
        logger.debug && logger.debug('MASTER  sticky header balancer headers:', req.headers);
        if (options.stickyHeader && req.headers[options.stickyHeader]) {
          logger.debug && logger.debug('MASTER  sticky header balancer match header ' + options.stickyHeader + ': ' + req.headers[options.stickyHeader]);
  ...
  function stickyCookieBalancer(connection) {
    ...
    logger.debug && logger.debug('MASTER  sticky cookie balancer receive connection from ' + signature);
    connection.resume();
    ...
    connection.once('data', function(buffer) {
      ...
      parser.onIncoming = function(req) {
        ...
        logger.debug && logger.debug('MASTER  sticky cookie balancer headers:', req.headers);
        logger.debug && logger.debug('MASTER  sticky cookie balancer cookie name:', options.stickyCookie && options.stickyCookie.name);
        ...
        logger.debug && logger.debug('MASTER  sticky cookie balancer for', address);
  ...
  function start() {
    ...
    serverStart(function() {
      if (logger.debug) {
        logger.debug('MASTER  ...started at port %d', PORT, process.pid);
